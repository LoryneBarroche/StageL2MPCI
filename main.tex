...
\documentclass[a4paper,10pt,oneside]{article} % taille papier, taille police, recto {type de document=article}

%------------------------------------------------------------------%
%           Chargement de packages pour compiler                   %
%------------------------------------------------------------------
...
\usepackage[bottom]{footmisc} %JE L'AI RAJOUTER POUR METTRE DES NOTES DE BAS DE PAGE
...
\geometry{body={17cm,25.7cm}} % on redéfinit la zone de texte
\rhead{L2 - MPCI} % haut de page à droite 
\lhead{Rapport de stage} % haut de page à gauche
 \cfoot{\thepage} % bas de page au milieu 
...
\fancypagestyle{progressstyle}{
  \fancyhf{} % vide tous les entêtes/pieds
  \renewcommand{\headrulewidth}{0pt} % pas de ligne
  %\fancyhead[C]{\progressbarDrake}
 % barre centrée dans l'en-tête
...
\addto\captionsfrench{\def\tablename{Tableau}} % pour définir le nom de tableau
%https://www.overleaf.com/project/638ef4dc23dc57ea7394d200

% pour les définitions et théorèmes encadrés

\usepackage[most]{tcolorbox}

\newtcbtheorem[number within=section]{Definition}{Définition}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
 boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{def}

\newtcbtheorem[use counter from=Definition]{Theoreme}{Théorème}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
 boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{thm}
  
\newtcbtheorem[use counter from=Definition]{Corollaire}{Corollaire}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
  boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{coro}
  
\newtcbtheorem[use counter from=Definition]{Propriete}{Propriété}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
  boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{prop}

\newtcbtheorem[use counter from=Definition]{Proposition}{Proposition}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
  boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{propo}
  
\newtcbtheorem[use counter from=Definition]{Axiome}{Axiome}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
  boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{ax}

\newtcbtheorem[use counter from=Definition]{Conjecture}{Conjecture}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
  boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{conj}
  
\newtcbtheorem[use counter from=Definition]{Lemme}{Lemme}
{colback=gray!5, colframe=gray!40, fonttitle=\bfseries, coltitle=black,
  boxrule=0.5pt, arc=2pt, left=5pt, right=5pt, top=5pt, bottom=5pt}{lem}
...
%\def\rapporteur#1{\def\@rapporteur{#1}}
...
\vskip 50\p@ 
\begin{center}
     \LARGE{ \@docu} \par  % type de rapport
\end{center}

\vskip 10 pt     

\vskip 1\p@ % écart haut page - titre
\begin{center}
    % pour faire les 2 lignes du haut
    \hrule 
    \vskip 1pt 
    \hrule
    \vskip 1pt
    {\Large \bfseries \strut \@title \strut}\par  % titre du rapport
    % pour faire les 2 lignes du bas
    \vskip 1pt
    \hrule
    \vskip 1pt
    \hrule
...
\vskip 20\p@ % écart titre - auteur
\begin{center}
    \Large \@author \par  % auteur
\end{center}

\vskip 10\p@ % écart auteur - classe
\begin{center}
    \Large \@classe \par  % classe
\end{center}

\vskip 0\p@ % écart classe - année
\begin{center}
    \Large \@date \par  % année universitaire
\end{center}
 

%\vskip 20\p@ % écart prof-rapporteur
%\begin{center}
%\large
%	Rapporteur du stage :\\
%	\vspace{-0.3cm}
%    \large \@rapporteur \par  % rapporteur(s)
%\end{center}
...
\docu{\textsc{\LARGE Thème}}
\fancyfoot[R]{} %ce qu'on écrit en bas à droite (ex : date)
\fancyfoot[L]{} %ce qu'on écrit en bas à gauche (ex : classe)
...
\begin{Definition}{Les tuiles de Wang en deux dimensions}{wang2D}
...
\begin{Definition}{Compatibilité des tuiles de Wang}{comp}
...
\begin{Definition}{Pavage de $\mathds{Z}^2$ avec des tuiles de Wang}{paveZ2}
...
\begin{Definition}{Validité d'un pavage de $\mathds{Z}^2$ avec des tuiles de Wang}{valideZ2}
...
\begin{Definition}{Pavage faiblement périodique et Vecteur de Périodicité}{FaP}
...
\begin{Definition}{Pavage fortement périodique}{FoP}
On dit que $P_{2D} : \mathds{Z}^2 \rightarrow \tau$ est fortement périodique ssi $\exists \Vec{u} = (x_u, y_u) \neq \Vec{0}$, $(x_u, y_u) \in \mathds{Z}^2, \exists \Vec{v} = (x_v, y_v) \neq \Vec{0}$, $(x_v, y_v) \in \mathds{Z}^2$, $\nexists r \in \mathds{R}$ tq $\Vec{u} = r\Vec{v}$, et $\forall x \in \mathds{Z}^2$, $\forall k_1 \in \mathds{Z}$, $\forall k_2 \in \mathds{Z}$, $P_{2D}(x+k_1\Vec{u}+k_2\Vec{v}) = P_{2D}(x)$.
\end{Definition}

\NiceMatrixOptions{renew-dots,renew-matrix}

\begin{Definition}{Jeu de tuiles de Wang}{}
...
\begin{Definition}{Compatibilité de deux tuiles de $T_{l,h}(\tau)$}{valideZ}
